...
void test(BridgeManager *manager,int maxWeight, int nbCars, int nbTrucks)
{
    ...
    for(int t = 0; t < nbCars; t++){
        ...
        threadsVoiture[t] = std::make_unique<PcoThread>(carTask, &checker, manager, t, 3);
    }
    for(int t = 0; t < nbTrucks; t++){
        logger() << "Création de la tâche camion " << t << std::endl;
        threadsCamion[t] = std::make_unique<PcoThread>(truckTask, &checker, manager,t, 3);
    }
...
TEST(BridgeSimple, standard) {
    ...
    ASSERT_DURATION_LE(5, {
                           ...
                           test(manager.get(), maxWeight, NB_THREADS_VOITURE, NB_THREADS_CAMION);
                       });
}
