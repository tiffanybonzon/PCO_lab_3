...
public:
    ...
    void check()
    {
        ...
        EXPECT_TRUE(executedBefore(5, 7));
        EXPECT_TRUE(executedBefore(6, 7));
        EXPECT_TRUE(executedBefore(3, 8));
        EXPECT_TRUE(executedBefore(7, 8));
    }
};
...
TEST(Sequentialite, Standard)
{
    ...
    threads[4]->join();
    threads[5]->join();
    threads[6] = std::make_unique<PcoThread>(t7);
    threads[6]->join();
    threads[7] = std::make_unique<PcoThread>(t8);
    threads[7]->join();
